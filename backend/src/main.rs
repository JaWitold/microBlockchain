...
async fn main() {
    ...
    loop {
        let evt = {
            select! {
                mine = mine_rcv.recv() => {
                    Some(p2p::EventType::Mine(mine.expect("mine exists")))
                }
            ...
            match event {
                ...
                p2p::EventType::Consume => {
                    ...
                    swarm
                        .behaviour_mut()
                        .floodsub
                        .publish(p2p::DATA_TOPIC.clone(), json.as_bytes());
                    if let Err(e) = swarm.behaviour_mut().mine_sender.send(data) {
                        error!("error sending response via channel, {}", e);
                    }
                }
                p2p::EventType::Mine(data) => {
                    println!("MINING: {}", data);
